...
| `V1073-RUNTIME-013` natural-boundary audit collector | Stable privacy-safe runtime projections plus passive boundary observations | **Structural.** The observation schema and authority boundary are designed, and Tier 22 supplies the core transition evidence, but no collector or runtime polling is implemented. | Implement the explicitly enabled read-only collector with initial identity, revision, Perk, clearing, and history-tail coverage. It may emit audit candidates only: no attachment, record construction, Perks navigation decision, input, or navigation suppression. Upgrade, survival, and visual-event components remain independently manifest-gated extensions and do not block the core collector. |
...
only the row-level evidence stated above; it does not implement polling,
same-round caches, runtime tail attachment, record construction, the audit
collector, or navigation suppression. Foreground freshness extensions, active
upgrades, survival timing and repeated-event merging, live tallies, and future
unknown `killedBy` values remain independently fail-closed work rather than
blockers for the core `V1073-RUNTIME-013` collector.
