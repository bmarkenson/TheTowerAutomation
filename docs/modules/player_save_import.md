...
| `V1073-RUNTIME-007` structural tail identity and complete `BattleHistory` More Stats projection | Source-ordered capped `battleHistory[<=30]`; exact newest 148-field `BattleHistoryEntry` shape | **Shortcut-ready** for structural activity continuity, and for semantic projection only when the cause is mapped. Retained saves prove mixed UTC/local DateTime kinds and capped rollover. The prior 21 UI-captured battles plus the Tier 22 terminal confirm the complete ordered 144-row projection within UI precision; malformed entries and unknown semantic causes fail closed independently. `adGemsThisRound` supplies Ad Gems. | The source-tagged structural identity now supplies Home, direct-Retry, and guarded replacement-process `RUNNING` activity continuity; unknown `killedBy` does not block those structural uses. Trust source order rather than cross-kind ticks; keep raw entries, arbitrary fields, and account data unpublished. Normal terminal attachment and record construction remain later work. |
| `V1073-RUNTIME-008` Game Over history serialization timing | Pre-run history tail, Game Over observation, post-run stable save | **Causal.** The known pre-battle tail changed in the immediate stable post-death save while the natural Tier 22 terminal was preserved, proving publication at the Game Over boundary without an exact timestamp. The first enabled Tier 19 run independently recorded clearing and tail publication before normal Retry. | Immediate observation requests at Game Over and Tournament terminal labels are implemented. `b137ea4` adds guarded same-session direct-Retry baseline rollover; its next ordinary end-to-end receipt confirmation remains pending. Receipt timing remains a latency bound, and terminal record capture stays on the UI path until that separate attachment is implemented and proven. |
| `V1073-RUNTIME-009` terminal history-tail attachment | Pre-boundary structural tail fingerprint plus newest post-boundary entry tier/time/wave | **Causal** for a candidate only. The pre-battle baseline changed at capped rollover to a newest Tier 22, wave 751, Boss entry whose complete semantic projection agreed with terminal tier/time/wave evidence. This proves tail causality, not normal completed-record attachment. | Activity Continuity accepts an advancing tail for a runtime-owned direct Retry and for the narrow replacement-process Current-run scope comparison after guarded active serialization. Neither path builds or attaches a completed record. The independent collector remains observation-only. A later record consumer must preserve terminal binding and must not let a terminal-only restart inherit Strategy or process-local tracker context. |
...
ordinary rollover receipt. The independent activity-continuity path now uses
the structural tail for initial Home, runtime-owned direct Retry, and a guarded
replacement-process Current-run comparison already at `RUNNING`; it still does
not construct or attach a normal save-derived record. Foreground freshness
extensions, active upgrades,
