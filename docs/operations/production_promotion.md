...
| Runtime Python, YAML, templates, or runtime-read assets | Stop automation before update; restart and smoke-test afterward. When Stop retained an exact owned battle, ordinary Start performs and waits for its fresh same-battle Attach automatically. A changed active battle remains Paused. An ended battle may use the fresh unbound terminal fallback, but is not a successful same-battle Attach. In every case, an agent that stopped a Running runtime must restore the prior Running posture after fresh owner/target/screen checks before declaring the deployment complete, unless a superseding operator or safety boundary blocks it. |
