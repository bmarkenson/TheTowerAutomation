...
| Runtime Python, YAML, templates, or runtime-read assets | Stop automation before update; restart and smoke-test afterward. When Stop retained an exact owned battle, ordinary Start performs and waits for a fresh forced-save Attach automatically. An equal active identity reattaches that battle; a later identity clears old battle-local state and attaches the successor. An ended battle may use the fresh terminal lifecycle fallback; exact full-tier battle-start-counter continuity may bind only its save report and is not a successful active-battle Attach. In every case, an agent that began its work from a Running runtime must carry that restoration posture across its own pre-Stop Pause and the replacement's startup Pause, then restore Running after fresh owner/target/screen checks before declaring the deployment complete, unless a newer operator Pause or safety boundary supersedes it. |
| Control surface or shared modules | Stop automation first when shared runtime code changes, preserving any eligible exact-battle handoff; stop/restart the control-surface service, then Start automation and require the forced-save active-battle Attach (same or later identity) or the appropriate explicit Paused outcome in smoke evidence. |
