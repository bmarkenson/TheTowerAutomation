...
roundsStartedThisTier[currentTier], roundSeed)`—is the only durable active-
battle adoption key. Runtime obtains it by forcing serialization at every
lifecycle boundary that needs current identity; it never waits for a natural
save or substitutes Battle History, elapsed time, visual similarity, or
`activity_scope_run_id`. `V1073-RUNTIME-020` adds a narrower inactive
terminal-save continuity proof from the complete battle-start counter epoch.
That proof may unlock only separately versioned, exact-battle/configuration-
bound report snapshots; it does not make replacement-process state evidence.
The activity scope remains optional log/report/presentation metadata. The sole runtime
...
is available. Destination passive reads cannot advance the guarded marks. The
Wave values remain progress diagnostics; they are not part of canonical battle
identity.

Before any ordinary battle-bound input resumes, the first forced identity save
on the selected destination must retain the exact active identity and target,
must expose the comparable progress claims, and must not regress the available
save-revision mark. `currentWave` may move backward because the destination PC
starts from the newest cloud save, which can lag the source PC's local battle;
that expected rollback does not change `ActiveRoundIdentity` and does not reject
the handoff. A different identity, target change, missing comparable claim, or
lower revision makes the failure sticky and creates the typed catastrophic
`save_continuity_lost` Pause. Unknown and unrelated modal routes remain
input-inert while proof is pending. A fresh Home/New Battle forced save proving
`round_active=false` instead retires the old active record and its guard; this
is the normal boundary when the prior battle has already ended. A retained
wave-only failure written by the earlier policy is accepted once on startup
only when its ordered receipt already proves that identity, destination,
revision availability, and revision non-regression passed.

This comparison proves same-battle destination continuity; it does not claim an
exact write time, require preservation of unsynchronized local wave progress,
or prove that a cloud upload occurred. A divergent destination snapshot whose
identity and revision happen to satisfy the guard is outside what the mapped
save can distinguish. The operator should still complete the game's supported
cloud-save synchronization before an active-battle move when available.
...
`30.00m/30.00m/39.00m`, Farm `30.00m/31.80m/37.20m`, the current Tournament
preset `98.38m/80.37m/60.00m`, and the retained prior Tournament tuple
`98.38m/87.16m/80.37m`. The current preset comes from a reviewed version-1101
pre-mutation save/UI observation.
Its `innerOrbDistance` and
`workshopOrbDistance` mapping centers use one decimal place and each accept an
absolute raw variance of `0.1`. A tuple can replace both live Range and
Distance UI when its semantic Range equals one `current_active_round`
calculation from the exact forced save that binds the running battle. A
nonmax active Range is point-in-time evidence rather than a round invariant,
but it is current for this one bound setup consumption. At a verified Home
boundary, an eligible mapped Orb requirement crosses the exact owned launch as
a running-refresh intent; a unique Home tuple accompanies it as deferred
evidence when available because its Range is `configured_out_of_round`. The
already-required forced active-battle identity save then recalculates Range from
`upgradeLevel[4]`, verifies the same active-round identity, and replaces the
deferred tuple only when the fresh Range/Orb combination uniquely matches the
configured requirement. That accepted value omits both Attack Range and
Distance Adjuster UI. A changed identity, target, mapping, requirement, Range,
or Orb tuple discards the conditional value and retains the complete guarded
UI fallback. `force_ui` and `comparison_audit` never carry this evidence.
Preset names do not participate. Overlapping tuple windows, unsupported
versions, and malformed dependencies also retain the complete UI route.
...
suppression. A terminal-only replay cannot infer Strategy or process-local
tracker evidence from replacement-process state under the
[current-process terminal-binding rule](../issues/resolved-2026.md#terminal-only-restart-attached-stale-strategy-and-perk-history-to-a-manual-battle).
A later narrow exception may bind the terminal save report when the
restart handoff and retained active record agree and the complete declared
per-tier battle-start counter vector is exactly equal in a non-regressing,
same-target inactive save. It may also restore the retained Strategy/report
snapshot, matching completed session-preflight evidence, and versioned
survival-activation checkpoint, because each has its own exact battle and
configuration binding. Process-only Perk, metric, wave, coin, speed, and
transient observation state remains excluded. One legacy exception exists when
the vector was never retained: while automation is exactly Paused on a fresh
Game Over, an explicit trusted-operator attestation may bind the retained Stop
handoff, exact battle identity, current runtime/PID/target generation, and
unchanged selected Strategy. It may reuse an independently durable Strategy
snapshot or backfill that snapshot only when the exact-battle completed
session-preflight fingerprint matches the currently loaded definition. The
attestation is fingerprinted, audited, report-only, and invalid after its live
runtime or target binding changes; it grants no input or active-battle adoption
authority. Without vector proof or that explicit receipt, the screen-policy
fallback remains unbound. History chronology alone is never promoted into
battle identity. Unknown future `killedBy` values still fail semantic
publication closed. Foreground save
...
### 2026-08-16 version-1101 survival refresh-timer audit

An ordinary Tier 19 run supplied a natural Second Wind activation without any
agent input or battle-boundary change. The visual tracker observed the active
status at approximately wave 5,359 at 05:58:12 PDT; Demon Mode followed at
approximately wave 5,634 at 06:16:19 PDT. Bounded exact-target reads
decoded only the normalized allowlisted fields in memory; no raw save or
decoded root was retained.

The pre-activation revision 50,630 was saved at wave 5,345 with count zero and
`994,653` waves remaining, an absolute inactive target of `999,998`. An earlier
same-run zero-count checkpoint had resolved to `1,000,000`, so that nominal
target is not used as an exact sentinel. Revision 50,631 was saved at wave
5,420 with Second Wind count one, research level 7, a 400-wave recharge, and
335 waves remaining: `5,420 + 335 - 400 = 5,355`. Revision 50,632 was saved at
wave 5,504 with the same count/research/recharge and 252 waves remaining:
`5,504 + 252 - 400 = 5,356`. Revision 50,633 independently repeated 5,356
from saved wave 5,574 and 182 waves remaining. After the natural Demon Mode
transition, revision 50,634 was saved at wave 5,653 with count one, its level-7
300-wave recharge, and 278 waves remaining, deriving candidate wave 5,631.

The adjacent candidates prove a save-derived activation range of 5,355–5,356,
four to three waves tighter than the independent visual approximation. The
initial Demon Mode candidate was likewise three waves earlier than its visual
approximation. The runtime therefore keys the evidence by cumulative count,
accumulates candidate minimum/maximum values across monotonic same-identity
checkpoints, and labels
the report **Save-derived (timer)** rather than **Exact**. Zero-count,
outside-recharge-window, malformed, regressed, wrong-identity, and unknown-
version evidence remains untimed. This audit grants no input, lifecycle,
navigation, terminal, or Strategy authority.
...
| `V1073-V1101-RANGE-001` effective Attack Range | Active total `upgradeLevel[4]` or inactive `upgradeWorkshopLevel[4]`; selected/researched Range lab; `cardActive[4]`/`cardLevel[4]`; exact Cannon Primary/Assist Range effects and efficiency inputs | **Shortcut-ready calculation for exact versions 1073 and 1101.** The versioned contract reproduces native binary32 stage order, Range-card levels 1–7, module effect IDs 19–24 and gates, the `0x3e23d70c` compression constant, final meter scaling, and `Single.ToString("f2")` half-up formatting. | Publish the normalized value independently for reuse. Outside a battle it is complete with `configured_out_of_round` scope. During a battle, a nonmax current Range is point-in-time rather than round-invariant evidence, so the standalone claim remains incomplete; the same exact forced active save may still supply it as the immediate dependency of one Orb setup consumption. Selected lab above researched level, malformed arrays/types/modules, duplicate or out-of-domain effects, and unlisted forward versions fail only this claim and dependents. |
| `V1073-CFG-016` Orb Distance | Calculated effective Attack Range plus one-decimal `innerOrbDistance` and `workshopOrbDistance` centers with absolute raw tolerance `0.1` | **Shortcut-ready, current-Range/unique-tuple-scoped** for the configured Farm tuples, the current `98.38m/80.37m/60.00m` Tournament tuple, and the retained prior `98.38m/87.16m/80.37m` Tournament tuple. A forced `current_active_round` save can authorize the whole point-in-time shortcut even when Range is nonmax. A verified Home tuple remains deferred only until the owned successor's already-required identity save resolves current Range. | One unique raw-tolerance match may omit duplicate observation UI when its semantic Range equals the current calculation in the exact forced active-round bundle; an enforced mismatch still uses the guarded repair route. Out-of-round evidence alone never supplies live Range. A changed identity, target, mapping, requirement, Range, or Orb tuple restores the existing guarded UI fallback. Preset names and `rangeLevelSelected` alone grant no authority. Overlap, out-of-range raw values, malformed dependencies, unsupported versions/requirements, `force_ui`, or audit retains the complete route. `savedWorkshopOrbDistance` is not authoritative, and tolerance never invents a new tuple. |
...
| `V1073-RUNTIME-002` atomic per-check suppression and exact-next-battle carry | Resolved configuration fingerprint, per-component decisions, runtime-owned launch, first stable `RUNNING` | **Shortcut-ready and implemented** for all currently allowlisted Home/session components together, including Damage Slider and Orb Distance. An eligible Orb requirement carries a single-use running-refresh intent, plus a Home-deferred tuple when available; the successor's exact forced active-round identity bundle can replace it with current Range/Orb evidence before consumption. | `force_ui` preserves complete UI behavior; `comparison_audit` collects normalized comparison evidence while UI remains authoritative. A trusted exact mismatch queues only its guarded UI path; an actual repair removes only that check and closes pre-action mapping correlation, while unrelated accepted decisions remain authoritative. Deferred, refresh-pending, and complete facts share the exact context, launch, binding, fallback, invalidation, and single-consumption guards. Changed target, identity, mapping, requirement, or active tuple rejects only the Orb shortcut and restores UI. Independently UI-verified sections retain explicit UI provenance. Global trust, battle-identity, and save/UI-contradiction failures reject carry; a requirement-specific failure rejects only its check. Future comparisons never self-promote a manifest. |
| `V1073-RUNTIME-003` active round identity | `(versionNumber, currentTier, roundsStartedThisTier[currentTier], roundSeed)` | **Causal.** A known Home boundary preceded the first stable Tier 22 active projection with a new per-tier counter and round seed; subsequent stable revisions retained that exact identity through wave 710. No finer wall-clock latency is claimed. | This is the only durable active-battle adoption key. Home must force an inactive proof before Start; active/Resume boundaries force this identity before adoption. Equal means same battle, unequal means later battle, and no History/UI/log-scope fallback exists. `V1073-RUNTIME-020` is a save-report-only terminal continuity exception, not an active adoption substitute. |
...
| `V1073-RUNTIME-015` survival-ability checkpoint state | Demon Mode, Nuke, and Second Wind `*UsedThisRound`, use-count, cooldown, `*WavesUntilRefresh`, active/effect-timeout, and timer fields | **Structural for version 1073; narrow version-1101 timer capability implemented.** Version 1101 publishes only the three plural use counts, three refresh countdowns, and versioned recharge-research dependencies. A captured active countdown derives a candidate with `saved_wave + waves_until_refresh - recharge_waves`; repeated candidates bound the save-derived wave range. Zero-count or outside-recharge-window timers remain untimed. Boolean polarity, effect timeouts, and the other timer fields remain unclaimed. | Save-derived evidence requires a positive count, known recharge level, plausible active recharge window, exact active-round identity, and monotonic checkpoint. The inactive target is diagnostic rather than an exact sentinel: ordinary reads produced both `1,000,000` and `999,998`. A missed recharge remains untimed and visual evidence remains approximate. Exact 1073 and unknown forward versions do not inherit this exact-version formula. |
| `V1073-RUNTIME-016` save-checkpoint and visual-tail event merge | Same-round stable revisions, normalized survival checkpoints, passive visual activation events, and terminal Battle History counts | **Partially implemented for active version-1101 checkpoints, including durable restart retention.** The run-scoped tracker accepts the shared typed bundle, rejects identity or count/checkpoint regression, keys save-timer evidence by the cumulative activation count, and merges a nearby visual transition without double counting even when visual-only numbering missed an earlier event. Repeated timer candidates widen only that activation's bounded wave/re-arm range; save-only captured activations are retained. Its versioned checkpoint preserves the report projection and monotonic save cursor under the exact active identity and its own tracker-configuration fingerprint, restores during a same-battle active attachment, and becomes terminal-report evidence only after `V1073-RUNTIME-020` vector proof or its explicit legacy operator-attestation exception. It is independent of Strategy, including for No Strategy battles. | Current reports label timer-backed waves or ranges **Save-derived (timer)** and visual-only waves **Approximate (visual)**. Terminal restoration is labeled **observed events through checkpoint**, never complete history; absent entries do not prove that no later activation occurred. An activation after the newest usable active checkpoint—especially when Game Over arrives before persistence or another in-battle save—has no refresh-timer evidence. A late snapshot exposes only its latest counted activation and does not invent missed intermediate timing. Post-clear completeness and terminal Battle History count reconciliation remain pending. Conflict, corruption, or missing identity/configuration binding omits the checkpoint without synthesizing a wave. |
...
| `V1073-RUNTIME-019` completed-run profile progression attachment and delta | Same-target-generation terminal stable save; versioned `profile_progression`; newest earlier normal battle snapshot | **Structural and implemented.** Exact-version synthetic coverage validates malformed-field isolation, private-field exclusion, source-index diffs, first-run baselines, prior-record selection, Markdown rendering, and target-generation discard. One bounded read-only live save normalized all 12 current components without retaining the raw save. | The same terminal read now also feeds report attachment, but profile progression stays global and nonblocking while the report independently requires active-run or `V1073-RUNTIME-020` counter continuity plus tail causality. A terminal-only process may attach global profile state and the specifically durable components named by `V1073-RUNTIME-020`; it cannot infer any other tracker state. |
| `V1073-RUNTIME-020` full-tier battle-start counter epoch | Complete declared `roundsStartedThisTier` vector plus game version, save revision, active-round identity record, Stop handoff, and exact target generation | **Structural and narrowly implemented.** Every exact-shape save publishes a privacy-safe fingerprint of all 40 nonnegative counters whether the round is active or inactive. A forced active binding retains that fingerprint, its tier count, save revision, and target binding. A replacement at fresh Game Over may bind the terminal save report when the handoff identity/target agree and the inactive vector is exactly equal at a non-regressing revision. Active and terminal evidence from the 2026-08-16 Tier 19 run both retained counter `319`, corroborating start-time rather than completion-time advancement. The active record also retains one immutable Strategy/run-configuration snapshot, a separately fingerprinted completed session-preflight receipt, and the `V1073-RUNTIME-016` activation checkpoint. A valid completed preflight receipt is independently restorable even if an older record has no Strategy snapshot; it supplies Strategy/preflight evidence but does not invent the missing run configuration. | Equality means no successor battle started after the retained active snapshot; any changed tier, shape drift, missing legacy evidence, target change, or revision regression rejects recovery and preserves UI fallback. This is not a replacement for canonical active-round adoption, and no pre-start “exactly one increment” relation grants authority. After equality, only independently validated components for the same identity and configuration are restored report-only. A trusted operator may explicitly bridge a legacy missing-vector terminal only under the fresh-Paused exact-runtime contract described above; the receipt records whether its Strategy snapshot was independently durable or operator-backfilled. Perk timelines, active-run metrics, wave/coin samples, speed control, and other replacement-process or incomplete tracker state are not inferred. |
...
binding and same-source tail advancement. Foreground freshness extensions,
active upgrades, survival terminal-count reconciliation and missed intermediate
events, remaining live tallies, and future unknown `killedBy` values remain
independently fail-closed work rather than blockers for configuration preflight.
