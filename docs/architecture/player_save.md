...
| `V1073-CFG-016` Orb Distance | Calculated effective Attack Range plus one-decimal `innerOrbDistance` and `workshopOrbDistance` centers with absolute raw tolerance `0.1` | **Shortcut-ready, current-Range/unique-tuple-scoped** for the configured Farm and Tournament tuples. A forced `current_active_round` save can authorize the whole point-in-time shortcut even when Range is nonmax. A verified Home tuple remains deferred only until the owned successor's already-required identity save resolves current Range. | One unique raw-tolerance match may omit all UI when its semantic Range equals the current calculation in the exact forced active-round bundle. Out-of-round evidence alone never supplies live Range. A changed identity, target, mapping, requirement, Range, or Orb tuple restores the existing guarded UI fallback. Preset names and `rangeLevelSelected` alone grant no authority. Overlap, out-of-range raw values, malformed dependencies, unsupported versions/requirements, `force_ui`, or audit retains the complete route. `savedWorkshopOrbDistance` is not authoritative, and tolerance never invents a new tuple. |
