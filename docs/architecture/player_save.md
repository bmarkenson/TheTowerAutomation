...
`activity_scope_run_id`. `V1073-RUNTIME-020` adds only a narrower inactive
...
A later narrow exception may bind only the terminal save report when the
...
same-target inactive save. The screen-policy fallback remains unbound if that
proof is absent or fails; History chronology alone is never promoted into
...
| `V1073-RUNTIME-016` save-checkpoint and visual-tail event merge | Same-round stable revisions, normalized survival checkpoints, passive visual activation events, and terminal Battle History counts | **Partially implemented for active version-1101 checkpoints.** The run-scoped tracker accepts the shared typed bundle, rejects identity or count/checkpoint regression, keys save-timer evidence by the cumulative activation count, and merges a nearby visual transition without double counting even when visual-only numbering missed an earlier event. Repeated timer candidates widen only that activation's bounded wave/re-arm range; save-only captured activations are retained. | Current reports label timer-backed waves or ranges **Save-derived (timer)** and visual-only waves **Approximate (visual)**. An activation after the newest usable active checkpoint—especially when Game Over happens before another in-battle save—has no refresh-timer evidence and remains visually approximate. A late snapshot exposes only its latest counted activation; it does not invent missed intermediate timing. Cross-process cache restoration, post-clear retention, and terminal Battle History count reconciliation remain pending. Conflict or missing binding retains existing evidence without synthesizing a wave. |
