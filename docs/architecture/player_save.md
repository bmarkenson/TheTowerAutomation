...
| `V1073-RUNTIME-012` forced terminal UI audit/fallback | Existing Game Stats, Perks, clipboard/OCR More Stats, and verified terminal controls | **Shortcut-ready and preserved.** Game Stats and Perks remain passive evidence; the More Stats clipboard/OCR route is now conditional. | Force More Stats on audit, unknown version/shape/cause, absent or incompatible baseline, unbound terminal, invalid tail transition, terminal-kind mismatch, or save-record contradiction. Wait/Retry/Home and mutation/transition confirmation always remain verified UI actions. Conditional Perks navigation remains future work. |
...
All remaining rollout, comparison, Perk checkpoint, active-upgrade, survival,
tally, and future-version work is owned by the
