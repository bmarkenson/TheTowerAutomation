...
transient observation state remains excluded. The screen-policy fallback
remains unbound if vector proof is absent or fails; History chronology alone is
never promoted into battle identity. Unknown future `killedBy` values still
fail semantic publication closed. Foreground save
