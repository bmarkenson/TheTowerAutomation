...
this tuple the only durable active-battle adoption key. `BattleIdentityStore`
atomically records the exact active identity, or a forced inactive Home proof,
and classifies a later active read as first, same, or later. Its retained
full-tier counter epoch may separately bind an inactive process-restart Game
Over save report; it cannot adopt an active battle. While a battle is active,
the same record freezes one content-fingerprinted Strategy/run-configuration
snapshot and advances one versioned survival-activation checkpoint. A matching
session-preflight receipt retains its own exact check-configuration fingerprint.
The activation checkpoint instead carries its own tracker-configuration
fingerprint and does not require a Strategy. After terminal vector proof, each
component is validated independently against that battle and its own
configuration before report-only restoration. The vector
never promotes replacement-process memory, and malformed optional evidence is
omitted without invalidating the core identity. Completed History, elapsed
time, visual activity, and log scope never substitute for either result.
...
process that starts on Game Over remains terminal-only and unbound by default.
Fresh runtime/target/screen evidence may nevertheless authorize that
replacement to follow the selected terminal policy. Wait retains Game Over
until its bounded hold expires, Home leaves it, and Continue dispatches Retry and arms the successor as a new
battle. This is lifecycle recovery only: it does not bind the completed battle
or make its save-derived report and process-local evidence reusable. The sole
legacy report-binding exception is a separately audited trusted-operator
attestation made while exact Pause and the full retained/live contract above
hold. That receipt expires with its named runtime or target generation and
never supplies input authority.
...
explicit `WAIT` holds its requested boundary until the ordinary idle deadline.
Game Over `WAIT` must first
...
- The persisted post-run selections are `NEXT_BATTLE`, `WAIT`, and `HOME`.
  ...
  terminal/Home boundary; `HOME` returns Home after Game Over. Their ordinary
