...
| `GAME_OVER` or `TOURNAMENT_RESULTS` | One lifecycle-bound `natural_boundary` bundle. A process-restart Game Over may propose the retained identity only when the Stop handoff, target generation, and durable full-tier counter baseline agree; the newly acquired inactive save must then prove exact vector equality and non-regressing revision. | Profile progression, structural terminal transition, semantic completed report, Perk-window closure, optional audit projection, and Tournament conditions. Recovered counter continuity also unlocks only independently durable component snapshots that are exact-battle and configuration bound: the immutable Strategy/report snapshot, a matching completed session-preflight receipt, and the versioned survival-activation checkpoint. Activation restoration is explicitly observed-events-through-checkpoint evidence, not a complete terminal history. | Projection or acquisition failure remains nonblocking and preserves the applicable Game Stats, Perks, or More Stats UI fallback. Missing legacy vector or component evidence, any counter change, shape drift, target change, revision regression, or component identity/configuration mismatch fails closed locally. Current-process-only Perk, metric, wave, coin, speed, and transient tracker state remains omitted. |
