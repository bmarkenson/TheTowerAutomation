...
- **Status:** Repair candidate implemented and passing the isolated development
  checkpoint; live
  deployment and confirmation are deliberately pending until the operator
  explicitly releases the active Tournament session. At that point reconcile
  current state from fresh read-only evidence first, then validate only at a
  naturally available no-battle boundary. Actionable confirmation remains in
  the [state/recovery backlog](../backlog/state-and-detection.md#state-coverage-and-recovery).
