...
- **Status:** Commit `ab84a3c` is deployed in production. Replacement-process
  ...
  [ISSUE-2026-032](#utility-dissonance-star-was-labeled-as-attack). The
...
**Stable ID:** `ISSUE-2026-032` · **Lifecycle:** `repair_awaiting_confirmation` · **Routing class:** `repair_awaiting_confirmation`

- **Observed:** 2026-08-06 at 18:13 PDT during the active Tier 19 No Strategy
  run. The complete 1080x1920 frame showed a purple circle with a white star
  immediately beside `Tier 19`; the operator identified it as Utility
  Dissonance.
- **Symptom:** `detect_attack_dissonance_badge()` counted purple pixels only.
  The star produced 1,061 pixels against its 500-pixel threshold, so the
  observer recorded `Attack Dissonance` and marked Damage Slider unavailable
  even though Utility Dissonance disables Utility systems and leaves Attack
  accessible.
- **Evidence:** A bounded exact-target screenshot retained the narrow 80x80
  badge regression crop. Two byte-identical read-only save reads from the same
  active Tier 19 round reported `dissonanceActive=true` and
  `dissonanceSelected=3`. Those raw values corroborate the visual observation
  but remain candidate evidence because their enum semantics and completed
  `dissonanceType` transition are not yet allowlisted.
- **Safety response:** Inspection sent no game input and did not Pause, Resume,
  Stop, replace, or Surrender the operator-owned battle. The existing runtime
  and its in-memory observation were left unchanged pending normal promotion.
- **Cause:** The code and documentation called the localized region an Attack
  sword detector, but implementation had no sword/icon check. Dissonance
  family color and subtype shape were collapsed into one boolean.
- **Repair:** The detector now records localized purple as Dissonance-family
  evidence and compares the white icon contour with the existing tab symbols.
  The historically established sword and the retained star are the only
  promoted subtypes; unvalidated shapes remain generic Dissonance. Utility no
  longer suppresses Damage Slider, `Util Disso` can recover identity after a
  post-run process replacement, and battle classification preserves the
  subtype label. Focused observer, inventory, classification, and battle-record
  coverage includes the live star crop, Attack-only constraint, generic
  fallback, post-run recovery, and terminal record label. The isolated
  checkpoint passed compilation, state-definition validation, clickmap
  integrity with zero errors and the 44 established orphan candidates, and all
  1,690 tests.
- **Status:** Repair commit `17e4e0c` is deployed through production merge
  `7029456` behind rollback tag
  `production-before-20260807T075200Z-cfbad10`. The exact candidate passed all
  1,700 tests; the replacement runtime acquired the exact target lock and
  published a fresh paused observation. The current Farm run was on the
  Event/Bots screen, so promotion left it Paused without input instead of
  claiming Utility evidence or resuming unowned navigation. Repair still
  awaits a fresh Utility running observation and the natural Game Over record.
  Require
  `battle_type=dissonance`, label `Utility Dissonance`, a guarded Attack Damage
  Slider read, and no subtype claim from raw save fields until
  `dissonanceSelected=3` is correlated with completed `dissonanceType`. Track
  confirmation in the
  [runtime backlog](../backlog/runtime-and-validation.md#runtime-control).
