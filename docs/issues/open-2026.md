...
- **Status:** Commit `ab84a3c` is deployed in production. Replacement-process
  attachment confirmed the save-only and unresolved-field behavior, but it did
  not reproduce the required paused `NEW_BATTLE -> RUNNING` transition. One
  natural paused manual-start confirmation remains tracked in the
  [runtime backlog](../backlog/runtime-and-validation.md#runtime-control).
