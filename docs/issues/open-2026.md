...
**Stable ID:** `ISSUE-2026-004` · **Lifecycle:** `confirmed_unresolved` · **Routing class:** `global_live_preflight_hazard`
...
- **Observed:** 2026-08-06 while starting an Attack Dissonance battle manually
...
- **Evidence:** The action log records scope
  ...
  Attack Dissonance screen. The run advanced from wave 20 through at least 510
...
- **Follow-on:** The repaired source transition exposed
  [ISSUE-2026-028](#running-attachment-used-battle-history-and-repeated-save-backed-configuration-ui):
...
- **Status:** Repair awaits integration, production promotion, and one natural
  paused manual-start confirmation that the running save baseline completes
  without Battle History UI and No Strategy visits only unresolved fields. The
  active confirmation is tracked in the
  [runtime backlog](../backlog/runtime-and-validation.md#runtime-control).

### Running attachment used Battle History and repeated save-backed configuration UI

**Stable ID:** `ISSUE-2026-028` · **Lifecycle:** `repair_awaiting_confirmation` · **Routing class:** `repair_awaiting_confirmation`

- **Observed:** 2026-08-06 after the paused manual-start source repair allowed
  the running Attack Dissonance attachment to advance. A replacement runtime
  started at 16:30:59 PDT and observed the same active battle near wave 1570.
- **Symptom:** With no persisted History baseline, the runtime opened Battle
  History instead of using `playerInfo.dat`. It subsequently navigated Cards,
  Perks, Ultimate Weapons, Modules, Bots, Guardians, and Target Priority even
  though the exact-version save already exposed complete normalized values for
  those supported fields. Attack Dissonance also made Damage Slider
  authoritatively inaccessible.
- **Evidence:** The action log selected the Battle History channel before the
  No Strategy inventory. The active save mapping
  `data-9-game-1073` allowlists selected Cards/Workshop/Bots presets, Free
  Upgrade locks, Guardian chips, Modules, Target Priority, Auto Pick state and
  configuration, and the complete Ultimate Weapon component set. The
  attachment reader previously returned only the structural History tail and
  discarded those normalized checks.
- **Safety response:** Diagnosis used current host-backed process/status, the
  action log, and bounded exact-target observation. It did not add a tap,
  Surrender, lifecycle transition, or control mutation to the operator-owned
  battle.
- **Cause:** The paused Home route changed its source to `RUNNING` but retained
  baseline mode; save selection also required an existing comparable baseline.
  A missing baseline therefore selected UI. Separately, the guarded serializer
  dropped validated profile checks at the History-reader boundary, and the No
  Strategy traversal visited every section unconditionally.
- **Repair:** Commit `cde691b` gives a missing running baseline its own
  attachment mode and makes every `save_first` `RUNNING` attachment save-only.
  A usable fresh tail records, compares, migrates, or conservatively replaces
  the scope; an unusable save waits without opening Battle History. The same
  exact snapshot projects only complete allowlisted checks into No Strategy.
  Its route visits only unresolved fields, the passive Dissonance badge resolves
  Damage Slider without probing Attack, and save-complete Home fields skip the
  post-run Workshop/Cards/Perks traversal. Focused save, continuity, No
  Strategy, lifecycle, and lease coverage passed 392 tests. The complete
  isolated checkpoint passed compilation, state and clickmap validation, and
  all 1,676 tests.
- **Status:** Repair awaits integration, production promotion, and one natural
  paused manual-start confirmation. Confirm one guarded save acquisition, no
  Battle History UI, no input while Paused, and only genuinely unresolved
  configuration UI afterward. Track that boundary in the
  [runtime backlog](../backlog/runtime-and-validation.md#runtime-control).

### Terminal development lease was replayed as an abnormal runtime change

**Stable ID:** `ISSUE-2026-029` · **Lifecycle:** `repair_awaiting_confirmation` · **Routing class:** `repair_awaiting_confirmation`

- **Observed:** 2026-08-06 at 16:31:00 PDT immediately after runtime
  replacement.
- **Symptom:** The new process emitted
  `Lease ended abnormally ... the production runtime/session changed` for lease
  `65557dd436e44ea7a4132258fb7b8bb6`, although the durable directive already
  recorded that lease as terminal with disposition `released`.
- **Evidence:** The lease's terminal release predated the replacement. The
  replacement runtime/session necessarily differed from the historical owner,
  but no active lease was being revoked and no new abnormal event occurred.
  The normal release path had already emitted its one `RESULT` describing the
  actual disposition.
- **Cause:** The runtime compared the lease's former process binding before it
  checked `request_state=terminal`. It therefore re-terminalized historical
  state through the abnormal branch and produced a duplicate, false warning.
- **Repair:** Commit `cde691b` recognizes the terminal directive first,
  preserves its recorded acknowledgement/disposition, removes any stale local
  hold, and emits no duplicate warning or result. A genuinely active lease
  whose runtime binding changes still terminates abnormally with its warning
  and paired result. Regression coverage exercises a released lease under a
  replacement runtime and proves no persistence rewrite, abnormal warning, or
  duplicate result. The shared complete checkpoint passed all 1,676 tests.
- **Status:** Repair awaits integration and observation across one ordinary
  process replacement with a retained terminal lease. The original release
  result should remain the only outcome; track confirmation in the
  [runtime backlog](../backlog/runtime-and-validation.md#runtime-control).
