...
- **2026-08-10 operator recurrence and recovery:** The operator reported that
  ...
  change.
- **Implemented mitigation:** Commit `040ca69` adds a default-off,
  exact-instance restart workflow in server revision 35 and the matching
  Windows client. It requires two
  depressed same-configuration Farm records at near-normal effective speed,
  sustained exact-run BlueStacks handle growth, no host CPU/memory saturation,
  a fresh runtime/battle-scope hold, and exact Windows listener
  PID/path/start-time acknowledgement. Linux then relaunches The Tower, handles
  the separate Welcome Back modal, suppresses its non-earning wave rollback,
  and starts a configured new battle after bounded Resume failure. Recovery
  records are excluded from later trigger calibration. The current contract is
  in the [control-surface](../architecture/control_surface.md#automatic-bluestacks-degradation-recovery)
