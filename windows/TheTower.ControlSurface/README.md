...
unsaved combo-box selection. The same panel selects the bundled strategy for
the next managed start; strategy changes are enabled only while automation is
completely stopped.
...
delay the log display. Select one or more rows and use **Copy selected**,
...
The ADB port, stopped-only strategy, and startup-gate policy share the managed
